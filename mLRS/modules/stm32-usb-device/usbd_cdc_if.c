...
static uint8_t usbd_dtr_rts = 0; // to track DTR RTS state
...
static int8_t CDC_Control(uint8_t cmd, uint8_t* pbuf, uint16_t length)
{
    ...
    case CDC_SET_CONTROL_LINE_STATE:
        ...
        if ((req->wValue & 0x0001) != 0) usbd_dtr_rts |= USBD_DTR_FLAG;
        if ((req->wValue & 0x0002) != 0) usbd_dtr_rts |= USBD_RTS_FLAG;
        break;
    }
